...
├── data/movies.ndjson              the catalogue, generated by ./start.sh
...
`start.sh` does exactly four things, and you can run any of them by hand:
...
| 3. Wait | Polls until Elasticsearch, Kibana and the 6,959-document catalogue are all ready. Every wait is bounded — on a timeout it tells you which `docker compose logs` to read instead of hanging. |
| 4. Data views | Creates the three Kibana data views (`elastiflix-movies`, `elastiflix-logs-*`, `metricbeat-*`), then prints the URLs. `allowNoIndex` is set because `metricbeat-*` genuinely does not exist yet on a first start. |
...
One helper, at the repo root:

| Script | For |
|---|---|
| `./generate-traffic.sh` | 54 searches (18 queries × 3 rounds), including deliberate zero-result ones, so the dashboard isn't empty. Pass a round count: `./generate-traffic.sh 10` |

Cache the images before recording with `docker compose pull && docker compose build`.
